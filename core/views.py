...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def proceed_to_checkout(self, request):
        ...
        try:
            ...
            """close any previous pending orders for the users/session to avoid duplicates"""
    ...
    if result.get('status') and result['data']['status'] == 'success':
        ...
        try:
            ...
            try:
                ...
                logger.exception("Failed to update order status in callback")
...
class GetUserOrder(APIView):
    """To get lists of orders the particular user has made, MyOrders.jsx"""
    def get(self, request):
        session_id = request.query_params.get('session_id')
        if not session_id:
            return Response({
        ...
        orders = Order.objects.filter(session_id=session_id)  #fetch all orders for the user
